...
fprintf('--- Controllo Dimensioni ---\n');
whos time pos position_des err_p dot_err_p err_R err_W u_T tau_b
fprintf('--------------------------\n');
...
grid on; box on;
exportgraphics(f, 'plothier/position_error.pdf');
...
xlim([min(time) max(time)])
grid on; box on;
exportgraphics(g, 'plothier/linear_velocity_error.pdf');
...
xlim([min(time) max(time)])
grid on; box on;
exportgraphics(h, 'plothier/orientation_error.pdf');
...
xlim([min(time) max(time)])
grid on; box on;
exportgraphics(y, 'plothier/angular_velocity_error.pdf');

%%  TOTAL THRUST u_T 
s = figure('Renderer','painters','Position',[10 10 900 700]);
plot(time, u_T, '-', 'Color', [0, 0.5, 0], 'LineWidth', 1.5); % u_T è 1xN
...
xlim([min(time) max(time)])
grid on; box on;
exportgraphics(s, 'plothier/thrust.pdf');

%%  TORQUE tau_b 
b = figure('Renderer','painters','Position',[10 10 900 700]);
hold on;
...
xlim([min(time) max(time)])
grid on; box on;
exportgraphics(b, 'plothier/torque_tau_b.pdf');

%% POSITION vs TIME 
o = figure('Renderer','painters','Position',[10 10 900 700]);
hold on;
...
xlim([min(time) max(time)])
grid on; box on;
exportgraphics(o, 'plothier/position.pdf');
